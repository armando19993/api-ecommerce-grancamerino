...
class OrderController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        return DB::transaction(function () use ($validated, $request) {
            ...
            foreach ($validated['items'] as $item) {
                $productVariant = ProductVariant::with('product')->findOrFail(
                ...
                $hasCustomization =
                    !empty($item['customization_name']) ||
                    !empty($item['customization_number']);

                if ($hasCustomization) {
                    // $5 USD por cada unidad personalizada

                    $customizationFee = 5.0 * $item['quantity'];
                ...
                $totalPrice =
                    $unitPrice * $item['quantity'] + $customizationFee;
                ...
                $orderItems[] = [
                    ...
                    'customization_fee' => $hasCustomization
            ...
            $shippingAmount = $totalQuantity === 1 ? 5.0 : 0.0;
            ...
            if ($validated['payment_gateway'] === 'wompi') {
                ...
                if (str_contains($redirectUrl, 'https://localhost')) {
                    ...
                    Log::warning(
                        'Wompi: Converted https://localhost to http://localhost',
                        [
                            'original_url' =>
                                $validated['redirect_url'] ?? 'config',
                ...
                Log::info('Wompi Checkout URL Generated', [
                    'url' => $checkoutUrl,
                ]);
                ...
                if ($cryptoCurrency) {
                    $nowPaymentData['pay_currency'] = strtolower(
                        $cryptoCurrency
                    );
                }
                ...
                if ($invoice) {
                    $paymentData = [
                        ...
                        'order_description' =>
                            $invoice['order_description'] ?? null,
                    ...
                    return response()->json(
                        [
                            'status' => 'error',
                            'message' =>
                                'Failed to create crypto payment. Please check the logs or try again later.',
                            'debug' => [
                                'api_key_configured' => !empty(
                                    config('services.nowpayments.api_key')
                                ),
                                'ipn_secret_configured' => !empty(
                                    config('services.nowpayments.ipn_secret')
                                ),
                                'base_url' => config(
                                    'services.nowpayments.url'
                                ),
                ...
                foreach ($orderItems as $item) {
                    ...
                    if (
                        $hasCustomization &&
                        isset($item['customization_fee']) &&
                        $item['customization_fee'] > 0
                    ) {
                        $stripeItems[] = [
                            'name' => "Personalización - {$item['product_name']}",
                            'description' =>
                                'Cargo por personalización de nombre/número',
                ...
                if ($session) {
                    ...
                    return response()->json(
                        [
                            'status' => 'error',
                            'message' =>
                                'Failed to create Stripe checkout session. Please try again later.',
                            'debug' => [
                                'secret_key_configured' => !empty(
            ...
            return response()->json(
                [
                    ...
                    'debug' => [
                        'signature_sent_to_wompi' =>
                            $paymentData['signature'] ?? null,
                        'reference' => $paymentData['reference'] ?? null,
                        'amount_in_cents' =>
                            $paymentData['amount_in_cents'] ?? null,
                        'currency' => $paymentData['currency'] ?? null,
                        'integrity_secret_configured' => !empty(
                            config('services.wompi.integrity_secret')
                        ),
                        'integrity_secret_length' => strlen(
                            config('services.wompi.integrity_secret') ?? ''
                        ),
                        'concatenation_formula' =>
    ...
    public function wompiWebhook(Request $request): JsonResponse
    {
        ...
        if (
            ...
            if ($order && $order->status === 'pending') {
                ...
                try {
                    $this->mailjetService->sendOrderStatusUpdate(
                        $order,
                        $oldStatus,
                        'paid'
                    );
            ...
            $event === 'transaction.updated' &&
            in_array($data['status'], ['DECLINED', 'ERROR', 'VOIDED'])
        ) {
            $order = Order::where('id', $data['reference'])->first();
            ...
            if ($order && $order->status === 'pending') {
                ...
                $order->update([
                    ...
                    'payment_data' => array_merge($order->payment_data ?? [], [
                        ...
                        'status_message' => $data['status_message'] ?? null,
                        'failure_reason' =>
                            'Payment ' . strtolower($data['status']),
                    ]),
                ]);
                ...
                Log::warning(
                    'Wompi: Order not found or not pending for failure',
                    [
                        'reference' => $data['reference'],
                        'order_status' => $order->status ?? 'not_found',
                    ]
            ...
            Log::info('Wompi: Event not processed', [
                ...
                'reason' =>
                    'Not a transaction.updated event or status not handled',
            ]);
        }
    ...
    public function nowPaymentsWebhook(Request $request): JsonResponse
    {
        ...
        if (
            !$this->nowPaymentsService->verifyIPNSignature($payload, $signature)
        ) {
        ...
        if (!$order) {
            Log::warning('NOWPayments: Order not found', [
                'order_id' => $orderId,
            ]);
    ...
    public function stripeWebhook(Request $request): JsonResponse
    {
        ...
        switch ($event['type']) {
            case 'checkout.session.completed':
                ...
                if (!$orderId) {
                    Log::warning(
                        'Stripe: No order_id in checkout.session.completed'
                    );
                ...
                if (!$order) {
                    Log::warning('Stripe: Order not found', [
                        'order_id' => $orderId,
                    ]);
                ...
                if (
                    ...
                    try {
                        ...
                        Log::error(
                            'Failed to send Stripe payment confirmation email',
                            [
                                'order_id' => $order->id,
                                'error' => $e->getMessage(),
                            ]
            ...
            case 'checkout.session.expired':
                ...
                if ($orderId) {
                    ...
                    if ($order && $order->status === 'pending') {
                        ...
                        Log::info(
                            'Stripe: Order marked as failed (session expired)',
                            [
                                'order_id' => $order->id,
                                'session_id' => $session['id'],
                            ]
                        ...
                        try {
                            ...
                            Log::error(
                                'Failed to send Stripe payment failure email',
                                [
                                    'order_id' => $order->id,
                                    'error' => $e->getMessage(),
                                ]
            ...
            case 'payment_intent.payment_failed':
                ...
                if ($orderId) {
                    ...
                    if ($order && $order->status === 'pending') {
                        ...
                        Log::info(
                            'Stripe: Order marked as failed (payment failed)',
                            [
                                'order_id' => $order->id,
                                'payment_intent' => $paymentIntent['id'],
                            ]
            ...
            case 'payment_intent.succeeded':
                ...
                if ($orderId) {
                    ...
                    if ($order && $order->status === 'pending') {
                        ...
                        Log::info(
                            'Stripe: Order marked as paid (payment_intent.succeeded)',
                            [
                                'order_id' => $order->id,
                                'payment_intent' => $paymentIntent['id'],
            ...
            default:
                Log::info('Stripe: Unhandled event type', [
                    'type' => $event['type'],
                ]);
        }
    ...
    public function checkPaymentStatus(
        ...
        if (
            $order->payment_gateway === 'wompi' &&
            $order->payment_data['transaction_id'] ?? null
        ) {
            $status = $this->wompiService->getTransaction(
                $order->payment_data['transaction_id']
            );
        } elseif (
            $order->payment_gateway === 'nowpayments' &&
            $order->payment_reference
        ) {
            $status = $this->nowPaymentsService->getPaymentStatus(
                $order->payment_reference
            );
        }
    ...
    public function testWompiSignature(Request $request): JsonResponse
    {
        ...
        return response()->json([
            ...
            'data' => [
                ...
                'instructions' => [
                    'frontend' =>
                        'Use checkout_url directly: window.location.href = response.data.checkout_url',
    ...
    public function updateStatus(Request $request, Order $order): JsonResponse
    {
        ...
        $validated = $request->validate([
            'status' =>
                'required|in:pending,paid,preparing,shipped,in_transit,delivered,cancelled,failed',
        ]);
